...
namespace WebServicesEnrollment.Services
{
    public class EnrollmentService : IEnrollmentService
    {
        ...
        public EnrollmentResponse EnrollmentProcess(EnrollmentRequest request)
        {
            ...
            if(aspirante == null)
            {
                respuesta = new EnrollmentResponse() {Codigo = 204, Respuesta = $"No existe el aspirante con el numero de expediente {request.NoExpediente}"};
        ...
        private Aspirante buscarAspirante(string noExpediente)
        {
            ...
            SqlDataAdapter adapter = new SqlDataAdapter($"select * from Aspirante a where a.NoExpediente = '{noExpediente}';",connection);
